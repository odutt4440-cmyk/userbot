...
# Lists directly imported from your paste
RAID = [
    ...
    "TERI CHUT KI CHUT MEI SHOULDERING KAR DUNG.AA HILATE HUYE BHI DARD HOGAAA😱🤮👺",
    "𝗧𝗘𝗥𝗜 𝗕𝗘𝗛𝗘𝗡 𝗞𝗜𝗧𝗡𝗜 𝗛𝗢𝗧 𝗛𝗔𝗜 𝗬𝗔𝗔𝗥 𝗠𝗔𝗡 𝗞𝗥𝗧𝗔 𝗛𝗔𝗜 𝗜𝗧𝗡𝗔 𝗖𝗛𝗢𝗗𝗨 𝗞𝗜 𝗔𝗨𝗥 𝗞𝗢𝗜 𝗖𝗛𝗢𝗗 𝗡𝗔 𝗣𝗔𝗬𝗘 𝗨𝗦𝗞𝗢😂😌🍆",
...
# State Trackers
SPAM_RUNNING = set() # (chat_id, sender_id)
REPLY_RAIDS = {} # {chat_id: {target_id: True}}

def register(client):

    # --- 1. SPAM COMMAND (.spam <count> <msg>) ---
    ...
    async def spam_handler(event):
        ...
        SPAM_RUNNING.add(session)
        
        await event.delete()
        for _ in range(count):
            if session not in SPAM_RUNNING: break
            await client.send_message(chat_id, text)
            await asyncio.sleep(0.3)
        
        if session in SPAM_RUNNING: SPAM_RUNNING.remove(session)

    # --- 2. RAID COMMAND (.raid <count> <target>) ---
    @client.on(events.NewMessage(outgoing=True, pattern=r"^\.raid (\d+)\s+(.*)"))
    async def raid_handler(event):
        ...
        for _ in range(count):
            if session not in SPAM_RUNNING: break
            msg = f"{mention} {random.choice(RAID)}"
            await client.send_message(chat_id, msg)
            await asyncio.sleep(0.5)
        
        if session in SPAM_RUNNING: SPAM_RUNNING.remove(session)

    # --- 3. REPLY RAID (.replyraid - Reply to someone) ---
    @client.on(events.NewMessage(outgoing=True, pattern=r"^\.replyraid"))
    async def reply_raid_set(event):
        if not event.is_reply:
            return await event.edit("❌ Reply to someone to start reply-raid.")
        
        reply = await event.get_reply_message()
        target_id = reply.sender_id
        chat_id = event.chat_id
        
        if chat_id not in REPLY_RAIDS: REPLY_RAIDS[chat_id] = {}
        REPLY_RAIDS[chat_id][target_id] = True
        
        await event.edit("✅ **Reply-Raid Activated!** Every message from them will be countered.")
    ...
    async def reply_engine(event):
        ...
        if chat_id in REPLY_RAIDS and sender_id in REPLY_RAIDS[chat_id]:
            await event.reply(random.choice(REPLYRAID))
